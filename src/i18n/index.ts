...
const zhMessages = {
	...
	"repl.thinkingLive": "思考中… {chars} 字",
	"repl.thinkingDone": "思考 {sec} · {chars} 字",
	"repl.expand": "Ctrl+O 展开",
	"repl.collapse": "Ctrl+O 折叠",
	"repl.expanded": "已展开最近一次思考。",
	"repl.noThinking": "没有可展开的思考内容。",
	"repl.copied": "已复制 {chars} 个字符到剪贴板。",
	"repl.copyFailed": "复制失败：未找到可用的剪贴板工具。",
	"repl.scrolledUp": "已上滚 {rows} 行（Ctrl+↓ 回到底部）",
	"repl.atBottom": "已回到底部。",
	"goto.title": "跳转到提示词",
	"goto.empty": "当前会话还没有发送过提示词。",
	"goto.hint": "↑/↓ 选择  ·  Enter 跳转  ·  Esc 取消",
	"goto.jumped": "已跳转到 #{index} {label}",
	"goto.cancelled": "已取消跳转。",
	"goto.outOfRange": "编号超出范围（共 {max} 条）。",
	...
	"toggle.thinkingView": "思考显示：{state}",
	...
	"tool.sumRead": "{path} · {lines} 行 · {size}",
	"tool.sumWrite": "{path} · {size}",
	"tool.sumList": "{path} · {count} 项",
	"tool.sumSearch": "{query} · {count} 处匹配",
	"tool.sumSearchNone": "{query} · 无匹配",
...
const enMessages: Record<MessageKey, string> = {
	...
	"repl.thinkingLive": "thinking… {chars} chars",
	"repl.thinkingDone": "thought for {sec} · {chars} chars",
	"repl.expand": "Ctrl+O to expand",
	"repl.collapse": "Ctrl+O to collapse",
	"repl.expanded": "Expanded the most recent thinking block.",
	"repl.noThinking": "No thinking block to expand.",
	"repl.copied": "Copied {chars} chars to clipboard.",
	"repl.copyFailed": "Copy failed: no clipboard tool available.",
	"repl.scrolledUp": "scrolled up {rows} lines (Ctrl+↓ for bottom)",
	...
	"goto.jumped": "Jumped to #{index} {label}",
	"goto.cancelled": "Jump cancelled.",
	"goto.outOfRange": "Index out of range (total {max}).",
	...
	"toggle.thinkingView": "thinking view: {state}",

	"tool.running": "→ running {name}: {detail}",
	"tool.ok": "✓ {name} done",
	"tool.fail": "✗ {name} failed: {error}",
	"tool.crashed": "Tool crashed: {error}",
	"tool.sumRead": "{path} · {lines} lines · {size}",
	"tool.sumWrite": "{path} · {size}",
	"tool.sumList": "{path} · {count} entries",
	"tool.sumSearch": "{query} · {count} matches",
	"tool.sumSearchNone": "{query} · no match",
	...
	"ui.loginOk": "signed in · token {len} chars",
	...
	"bar.hint": "! shell  ·  / commands  ·  Ctrl+T thinking  ·  Ctrl+S search  ·  Ctrl+C cancel",
