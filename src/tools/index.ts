...
export function buildToolDoc(lang: Lang): string {
	if (lang === "en") {
		return [
			...
			"- You may emit several calls at once (one per line) and they run in parallel; batch independent reads/searches to save round trips:",
			"  read:package.json",
			"  read:tsconfig.json",
	...
	return [
		...
		"注意：",
		"- 工具调用必须独占一行。",
		"- 可以一次给出多个调用（每行一个），它们会并行执行。互不依赖的读取/搜索请合并成一批，减少往返：",
		"  read:package.json",
		"  read:tsconfig.json",
