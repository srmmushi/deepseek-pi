...
❯ 读取 src/index.ts                          ← 自研行编辑器（历史/光标/CJK 宽度）
  ...
  ⏺ read(tsconfig.json)
  ⎿  read · package.json (7ms)               ← 结果按「完成顺序」打印
  ⎿  read · tsconfig.json (5ms)              ← 多工具时带工具名，避免归属歧义
  · 2 个工具并行  ·  8ms                     ← 批次汇总（串行需 12ms）
...
- **尺寸自适应**：终端缩放 / 全屏时重建滚动区域，并把光标**重新锚定到底部**，
  面板与输入行一起落到新位置——不会出现「输入行跑到顶部、新输出覆盖旧内容」。
...
- **工具配色**：`read` 青 · `write` 绿 · `list` 蓝 · `exec` 黄 · `search` 品红，类型一眼可辨。
